...
class FunkinLua
{
	public static var activeScripts:Array<FunkinLua> = [];
	...
	public function new(scriptName:String)
	{
		...
		var mainMenu:MainMenuState = MainMenuState.instance;
		if (mainMenu != null)
			mainMenu.luaArray.push(this);
		...
		set('version', "1.0.4");
		set('psychEngine_version', "1.0.4");
		set('dancoEngine_version', MainMenuState.dancoEngineVersion.trim());
		...
		if (mainMenu != null)
		{
			set("menu_allowMouse", mainMenu.allowMouse);
			
			set("menu_centerOptions", mainMenu.centerOptions);
			set("menu_curColumn", mainMenu.curColumn);

			set("menu_leftItem", mainMenu.leftItem);
			set("menu_leftOption", mainMenu.leftOption);

			set("menu_magentaBG", mainMenu.magenta);
			set("menu_menuItems", mainMenu.menuItems);

			set("menu_rightItem", mainMenu.rightItem);
			set("menu_rightOption", mainMenu.rightOption);

			set("menu_showOutdatedWarning", mainMenu.showOutdatedWarning);
		}
		...
		Lua_helper.add_callback(lua, "getRunningScripts", function()
		{
			...
			for (script in mainMenu.luaArray)
				runningScripts.push(script.scriptName);
		...
		Lua_helper.add_callback(lua, "callScript", function(luaFile:String, funcName:String, ?args:Array<Dynamic> = null)
		{
			...
			if (luaPath != null)
			{
				...
				for (luaInstance in mainMenu.luaArray)
					if (luaInstance.scriptName == luaPath)
						return luaInstance.call(funcName, args);
			}
